...
///
/// github: https://github.com/TangledHusky/TestOC/blob/master/TestOC/TKImageView
...
/**
 裁剪按钮代理事件
 */
@protocol LImageViewDelegate <NSObject>
- (void)LImageViewFinish:(UIImage *)cropImage; ///完成裁剪
- (void)LImageViewCancel;      ///取消裁剪
@end

typedef void(^ReturnImgBlock)(UIImage *image);

@interface LClipImgView : UIView
///需要裁剪的背景图
...
///裁剪图片
@property (nonatomic, copy) ReturnImgBlock getImgBlock;
...
/* ----------------- 代理事件--------------------------- */
///裁剪按钮delegate
@property (nonatomic, assign)id<LImageViewDelegate>delegate;
@end
