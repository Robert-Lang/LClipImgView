...
typedef NS_ENUM(NSInteger, LCropAreaCornerPosition) {
    LCropAreaCornerPositionTopLeft,
    LCropAreaCornerPositionTopRight,
    LCropAreaCornerPositionBottomLeft,
    LCropAreaCornerPositionBottomRight
};
typedef NS_ENUM(NSInteger, LMidLineType) {
...
@property (assign, nonatomic) LCropAreaCornerPosition cornerPosition;
...
- (void)setCornerPosition:(LCropAreaCornerPosition)cornerPosition {
...
- (void)drawCornerLines {
    ...
    switch (_cornerPosition) {
        case LCropAreaCornerPositionTopLeft: {
        ...
        case LCropAreaCornerPositionTopRight: {
        ...
        case LCropAreaCornerPositionBottomLeft: {
        ...
        case LCropAreaCornerPositionBottomRight: {
...
@property (strong, nonatomic) UIColor *lineColor;
@property (assign, nonatomic) LMidLineType type;
@end
...
- (void)setType:(LMidLineType)type {
...
- (void)drawMidLine {
    ...
    switch (_type) {
        case LMidLineTypeTop:
        case LMidLineTypeBottom: {
        ...
        case LMidLineTypeRight:
        case LMidLineTypeLeft: {
...
@property (assign, nonatomic) BOOL showSplitLines;
...
-(void)setShowSplitLines:(BOOL)showSplitLines
{
    _showSplitLines = showSplitLines;
    [self resetBorderLayerPath];
}
...
- (void)resetBorderLayerPath {
    ...
    CGFloat xy = _borderWidth / 2.0f;
    CGFloat w = WIDTH(self) - _borderWidth;
    CGFloat h = HEIGHT(self) - _borderWidth;
    if (self.showSplitLines) {
        layerPath = [UIBezierPath bezierPath];
        [layerPath moveToPoint:CGPointMake(xy, xy)];
        [layerPath addLineToPoint:CGPointMake(w, xy)];
        [layerPath addLineToPoint:CGPointMake(w, h)];
        [layerPath addLineToPoint:CGPointMake(xy, h)];
        [layerPath addLineToPoint:CGPointMake(xy, xy)];

        [layerPath moveToPoint:CGPointMake(w / 3, xy)];
        [layerPath addLineToPoint:CGPointMake(w / 3, h)];
        
        [layerPath moveToPoint:CGPointMake(w * 2 / 3, xy)];
        [layerPath addLineToPoint:CGPointMake(w * 2 / 3, h)];
        
        [layerPath moveToPoint:CGPointMake(xy ,h / 3)];
        [layerPath addLineToPoint:CGPointMake(w , h / 3)];
        
        [layerPath moveToPoint:CGPointMake(xy, h * 2 / 3)];
        [layerPath addLineToPoint:CGPointMake(w, h * 2 / 3)];

        [layerPath stroke];
    } else {
        layerPath = [UIBezierPath bezierPathWithRect: CGRectMake(xy, xy, w, h)];
    }
...
- (void)setUp {
    ...
    _cropAreaCornerWidth = 26;
    _cropAreaCornerHeight = 26;
    _cropAspectRatio = 0;
    _minSpace = 10;
    ...
    _cropMaskView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:_maskAlpha];
...
- (void)createCropButton {
    ...
    [_btnOK setBackgroundImage:[UIImage imageNamed:@"icon_gou"] forState:UIControlStateNormal];
    ...
    [_btnCancel setBackgroundImage:[UIImage imageNamed:@"icon_cha"] forState:UIControlStateNormal];
...
- (void)createCorners {
    ...
    _topLeftCorner.cornerPosition = LCropAreaCornerPositionTopLeft;
    ...
    _topRightCorner.cornerPosition = LCropAreaCornerPositionTopRight;
    ...
    _bottomLeftCorner.cornerPosition = LCropAreaCornerPositionBottomLeft;
    ...
    _bottomRightCorner.cornerPosition = LCropAreaCornerPositionBottomRight;
...
- (void)createMidLines {
    ...
    _topMidLine.type = LMidLineTypeTop;
    ...
    _bottomMidLine.type = LMidLineTypeBottom;
    ...
    _leftMidLine.type = LMidLineTypeLeft;
    ...
    _rightMidLine.type = LMidLineTypeRight;
...
-(void)setGetImgBlock:(ReturnImgBlock)getImgBlock
{
    _getImgBlock = getImgBlock;
    self.getImgBlock(self.currentCroppedImage);
}
-(void)setShowSplitLines:(BOOL)showSplitLines{
    _showSplitLines = showSplitLines;
    self.cropAreaView.showSplitLines = _showSplitLines;
}
#pragma mark - btn click
- (void)clickOkBtn{
    if (self.delegate && [self.delegate respondsToSelector:@selector(LImageViewFinish:)]) {
        [self.delegate LImageViewFinish:self.currentCroppedImage];
    }
}
- (void)clickCancelBtn{
    if (self.delegate && [self.delegate respondsToSelector:@selector(LImageViewCancel)]) {
        [self.delegate LImageViewCancel];
    }
}
-(void)getImgBlock:(ReturnImgBlock)block
{
    self.getImgBlock = block;
    if (block) {
        block(self.currentCroppedImage);
    }
}
